        ...
        <div className="flex-1 flex flex-col">
          ...
          <main className="flex-1 overflow-auto">
            <div className="px-6 py-8">
                ...
                <Card className="bg-gradient-to-br from-white/95 to-emerald-50/30 backdrop-blur-sm shadow-elegant border border-white/20 hover:shadow-2xl hover:shadow-emerald-100 transition-all duration-300 group cursor-pointer hover-scale animate-fade-in" style={{animationDelay: '100ms'}}>
                  <CardContent className="p-6">
                    ...
                    <p className="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent transition-all duration-300 group-hover:from-green-600 group-hover:to-emerald-600 group-hover:scale-105 transform">
                      ₹{getBankWiseData("all").amount.toLocaleString()}
                    </p>
              ...
              {/* Legal Opinion Applications Section */}
              <div className="mt-8 animate-fade-in" style={{animationDelay: '600ms'}}>
                <Card className="bg-gradient-to-br from-white/95 to-indigo-50/30 backdrop-blur-sm shadow-elegant border border-white/20 hover:shadow-2xl hover:shadow-indigo-100 transition-all duration-300">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        <CardTitle className="flex items-center space-x-2 group">
                          <Scale className="h-5 w-5 text-indigo-600 transition-all duration-300 group-hover:text-purple-600 group-hover:rotate-12 group-hover:scale-110" />
                          <span className="bg-gradient-to-r from-slate-800 to-indigo-600 bg-clip-text text-transparent transition-all duration-300 group-hover:from-indigo-600 group-hover:to-purple-600">Legal Opinion Applications</span>
                        </CardTitle>
                        <CardDescription className="text-slate-600 transition-colors duration-200 group-hover:text-indigo-600">Manage and assign legal opinion applications to employees</CardDescription>
                      </div>
                      <Button 
                        onClick={() => navigate('/admin/applications')}
                        className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg hover:shadow-xl transition-all duration-300"
                      >
                        Open Legal Opinion Dashboard
                      </Button>
                    </div>
                          ...
                          <TableBody>
                            {filteredApplications.map((application, index) => (
                              ...
                              >
                                ...
                                <TableCell>{application.bank_name}</TableCell>
                                <TableCell>{application.borrower_name}</TableCell>
                                <TableCell>{application.loan_type}</TableCell>
                                <TableCell className="font-semibold">
                                  ₹{parseFloat(application.loan_amount).toLocaleString()}
                                </TableCell>
                                <TableCell className="text-sm">
                                  {format(new Date(application.created_at), 'dd/MM/yyyy HH:mm')}
                                  <div className="text-xs text-slate-500">By: {application.submitted_by}</div>
                                </TableCell>
                                <TableCell>
                                  <Badge 
                                    variant={
