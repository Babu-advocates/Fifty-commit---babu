...
const LoanApplications = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      if (isEmployeeLogin && employeeUsername && !isAdminRoute) {
        ...
        query = query.in('status', ['to_be_assigned', 'in_review', 'under_review']);
      }
  ...
  const addDigitalSignatureToPDF = async (file: any) => {
    try {
      ...
      if (file.url) {
        ...
        if (signedUrlData?.signedUrl) {
          ...
          throw new Error('Could not access PDF file');
        }
      } else {
      ...
      const lineHeight = 12;
      const padding = 8;
      const maxLineWidth = Math.max(...signatureLines.map(line => font.widthOfTextAtSize(line, fontSize)));
      const blockWidth = maxLineWidth + (padding * 2);
      const blockHeight = (signatureLines.length * lineHeight) + (padding * 2);
      ...
      signatureLines.forEach((line, index) => {
        const isName = index === 1; // Second line is the name
        lastPage.drawText(line, {
          x: x,
          y: y + (blockHeight - padding - (index + 1) * lineHeight),
          size: fontSize,
  ...
  const handleSubmitOpinion = async () => {
    try {
      ...
      for (const file of opinionFiles) {
        if (file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf')) {
          try {
            ...
            const signedPdfBytes = await addDigitalSignatureToPDF(file);
            ...
            const signedFilePath = `${selectedApplication.id}/${signedFileName}`;
      ...
      // Update application with signed files and submitted status
      const { error: updateError } = await supabase
        .from('applications')
        .update({ 
          status: 'submitted',
  ...
  const filteredApplications = loanApplications.filter(app => {
    ...
    return matchesSearch && matchesBank && matchesStatus && matchesType && isNotSubmitted;
  });
              ...
              <Select value={selectedBank} onValueChange={setSelectedBank}>
                ...
                <SelectContent>
                  ...
                  {bankAccounts.map((account) => (
                    <SelectItem key={account.username} value={account.bank_name}>
                      {account.bank_name}
                    </SelectItem>
                  ))}
                </SelectContent>
